  ...
  return (
    <div className="pointer-events-none fixed left-1/2 top-20 z-50 flex -translate-x-1/2 flex-col items-center gap-2">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className="animate-toast-in whitespace-nowrap rounded-full border border-white/15 bg-black/85 px-5 py-2 text-base text-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.5)] [font-family:var(--font-card-title)]"
        >
          {toast.text}
        </div>
      ))}
